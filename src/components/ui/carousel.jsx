...
const Carousel = React.forwardRef((
  ...
  const [crousalIndex, setCrousalIndex] = React.useState(0);
  ...
  const onSelect = React.useCallback((api) => {
    ...
    setCrousalIndex(api.selectedScrollSnap())
  ...
  const handleWheel = React.useCallback((event) => {
    event.preventDefault();
    if (event.deltaY < 0) {
      scrollPrev(); // Scroll left/up
    } else {
      scrollNext(); // Scroll right/down
    }
  }, [scrollPrev, scrollNext]);
  ...
  return (
    (<CarouselContext.Provider
      ...
      <div
        ...
        data-crousal-index={crousalIndex}
        {...props}>
        {children}
      </div>
    </CarouselContext.Provider>)
